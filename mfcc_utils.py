...
def data_hparams():
    params = tf.contrib.training.HParams(
        # vocab
        data_type = 'train',
        data_path = 'C:/Users/asus/Desktop/语音识别/语音识别/data/',
        thchs30 = False,
        aishell = False,
		prime = False,
		jdz = True,
...
class get_data():
	...
	def get_am_batch(self):
		...
		for i in range(len(self.wav_lst)//self.batch_size):
			...
			for index in sub_list:
				pad_fbank = compute_mfcc(self.data_path + self.wav_lst[index])  # mfcc作为输入
				#fbank = compute_fbank(self.data_path + self.wav_lst[index])  # 声谱图作输入
				...
				# print(label_ctc_len)
				# print(pad_fbank.shape[0])
				if pad_fbank.shape[0]//1 >= label_ctc_len:
				#for i in range(0,49):################################### 待做
	...
	def wav_padding(self, wav_data_lst):
		...
		wav_lens = np.array([leng for leng in wav_lens])
		# new_wav_data_lst = np.zeros((len(wav_data_lst), wav_max_len, 200, 1))
		for index in range(0,778):
			pad_fbank = compute_mfcc(self.data_path + self.wav_lst[index])
			pad_fbank = int(pad_fbank.shape[1])
			#new_wav_data_lst = np.zeros((len(wav_data_lst), wav_max_len, pad_fbank, 1)) # pad_fbank][1]
		new_wav_data_lst = np.zeros((len(wav_data_lst), pad_fbank, wav_max_len,  1))
			#for i in range(len(wav_data_lst)):
			#new_wav_data_lst[index, :wav_data_lst[index].shape[0], :, 0] = wav_data_lst[index]
...
def compute_mfcc(file):
	...
	# mfcc_feat = librosa.feature.mfcc(y, sr, n_mfcc=13)
	# return mfcc_feat
